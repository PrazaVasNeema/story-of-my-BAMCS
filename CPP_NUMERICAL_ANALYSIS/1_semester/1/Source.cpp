...
void main() {
	int N;
	double a[20][20], c, s, temp_a, line_swap;
	string num;
	cout << "0, 1, 2, 3: ";
	cin >> num;
	...
	for (int k = 0; k < N; k++) {
		...
		for (int i = k + 1; i < N; i++) {
			c = a[k][k] / sqrt(a[k][k]*a[k][k]+a[i][k]*a[i][k]);
			s = a[i][k] / sqrt(a[k][k] * a[k][k] + a[i][k] * a[i][k]);
			for (int j = k; j < N; j++) {
				temp_a = a[k][j];
				a[k][j] = c * a[k][j] + s * a[i][j];
				a[i][j] = -s * temp_a + c * a[i][j];
				if (abs(a[k][j]) < 0.0001)
					a[i][j] = 0;
				if (abs(a[i][j]) < 0.0001)
					a[i][j] = 0;
			...
			temp_a = a[k][N];
			a[k][N] = c * a[k][N] + s * a[i][N];
			a[i][N] = -s * temp_a + c * a[i][N];
			if (abs(a[k][N]) < 0.0001)
				a[i][N] = 0;
			if (abs(a[i][N]) < 0.0001)
				a[i][N] = 0;
