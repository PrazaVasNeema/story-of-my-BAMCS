...
void main() {
	...
	cin >> razb;
	ofstream file("C://Limbo//Purgatory//To_wf//3//primer_" + to_string(primer) + ".txt");
	...
	case 1:
		...
		for (int i = 0; i < razb; i++) {
			...
			k[1] = primer1(t[i] + h / 3, y[0][i] + h * k[0][0] / 3, y[1][i] + h * k[0][1] / 3, y[2][i] + h * k[0][2] / 3);
			k[2] = primer1(t[i] + 2 * h / 3, y[0][i] + 2 * h * k[1][0] / 3, y[1][i] + 2 * h * k[1][1] / 3, y[2][i] + 2 * h * k[1][2] / 3);
			for (int j = 0; j < n; j++) {
				y[j][i+1] = y[j][i] + h / 4 * (k[0][j] + 3 * k[2][j]);
				file << "\t" << y[j][i+1] << '\t';
				cout << "\t" << y[j][i+1] << '\t';
			}
			file << endl;
	...
	case 2:
		...
		for (int i = 0; i < razb; i++) {
			...
			k[1] = primer2(t[i] + h / 3, y[0][i] + h * k[0][0] / 3, y[1][i] + h * k[0][1] / 3);
			k[2] = primer2(t[i] + 2 * h / 3, y[0][i] + 2 * h * k[1][0] / 3, y[1][i] + 2 * h * k[1][1] / 3);
			for (int j = 0; j < n; j++) {
				y[j][i + 1] = y[j][i] + h / 4 * (k[0][j] + 3 * k[2][j]);
				file << "\t" << y[j][i + 1] << '\t';
				cout << "\t" << y[j][i + 1] << '\t';
	...
	case 3:
		...
		for (int i = 0; i < razb; i++) {
			...
			k[1] = primer3(t[i] + h / 3, y[0][i] + h * k[0][0] / 3, y[1][i] + h * k[0][1] / 3);
			k[2] = primer3(t[i] + 2 * h / 3, y[0][i] + 2 * h * k[1][0] / 3, y[1][i] + 2 * h * k[1][1] / 3);
			for (int j = 0; j < n; j++) {
				y[j][i + 1] = y[j][i] + h / 4 * (k[0][j] + 3 * k[2][j]);
				file << "\t" << y[j][i + 1] << '\t';
				cout << "\t" << y[j][i + 1] << '\t';
